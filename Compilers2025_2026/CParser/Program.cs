...
namespace CParser {
    ...
    public class Program {
        static void Main(string[] args) {
            ...
            try
            {
                ScopeBuilderVisitor scopeVisitor = new ScopeBuilderVisitor();
                scopeVisitor.Visit(anltrst2AstGenerationVisitor.Root, new ParentInfo(null));

                // ΕΔΩ ΒΓΑΖΟΥΜΕ ΤΟ ΓΡΑΦΗΜΑ ΤΩΝ SCOPES!
                CScopeSystem.GetInstance().ScopeDebugLog("scopes.dot", "scopes.gif");

            }
            catch (Exception ex)
            {
                Console.WriteLine("ΣΦΑΛΜΑ ΣΤΟ SCOPE BUILDER: " + ex.Message);
            }
        }
